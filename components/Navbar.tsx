    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ${
          scrolled
            ? "bg-surface/92 backdrop-blur-xl border-b border-outline-variant shadow-[0_2px_20px_rgba(0,0,0,0.04)]"
            : ""
        }`}
      >
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="md:hidden fixed top-[72px] left-0 right-0 bg-surface/97 backdrop-blur-xl border-b border-outline-variant px-8 py-6 flex flex-col gap-5 z-[99]">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
